...
void VetorContas::mostraContasCorrente()
{
    for(int i=0; i<tamanho; i++)
    {
        if(dynamic_cast <ContaCorrente *> (elementos[i]) !=NULL)
            cout << elementos[i]->toString() << endl;
    }
}
void VetorContas::mostraContasPoupanca()
{
    for(int i=0; i<tamanho; i++)
    {
        if(dynamic_cast <ContaPoupanca *> (elementos[i]) !=NULL)
            cout << elementos[i]->toString() << endl;
    }
}
